...
export async function storeApiKey({
  ...
  const certification = await CertificationModel.findOneAndUpdate(
    { userId, service: "health.maruyu.work", resourceKey: "apiKey" },
    { data: { tokenType: "Bearer", apiKey, scope:"" }},
    { upsert: true, new: true }
  ).catch(error=>{
    console.error(error);
    throw new InternalServerError("error while updating health.maruyu.work apiKey. A");
  });
  if(!certification) throw new InternalServerError("error while updating health.maruyu.work apiKey. B");
}
...
export async function getStoredApiKey({
  ...
  const certification = await CertificationModel.findOne({userId, service:"health.maruyu.work", resourceKey:"apiKey"}).lean();
